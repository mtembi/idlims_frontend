...
import {fetchCustDeliveryRequest, showCustTxnDialog} from "../../redux";
import CustDelReqDialog from "./CustDelReqDialog";

const CustDelReqTable = () => {
    const gridRef = useRef(null);
    const dispatch = useDispatch();

    const delDataList = useSelector(state => state.custTxnFxnReducer.fetchCustDelRqList);

    useEffect(() => {
        dispatch(fetchCustDeliveryRequest());
    }, [dispatch]);
...
export default CustDelReqTable;